...
module SiteimproveAPIClient
  class LanguagePage
    ...
    # List of all languages for the page.
    attr_accessor :all_languages

    # Is true if language page is being checked now; otherwise false.
    ...
    # Percentage of the page written in the language.
    ...
    def self.openapi_types
      {
        ...
        :'title' => :'String',
        :'url' => :'String',
        :'all_languages' => :'Hash<String, Float>',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SiteimproveAPIClient::LanguagePage` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SiteimproveAPIClient::LanguagePage`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'all_languages')
        if (value = attributes[:'all_languages']).is_a?(Hash)
          self.all_languages = value
        end
      end
    ...
    def hash
      [id, title, url, all_languages, checking_now, cms_url, language_code, page_level, percentage, _siteimprove].hash
    end
