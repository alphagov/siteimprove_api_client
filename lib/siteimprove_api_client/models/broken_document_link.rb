...
module SiteimproveAPIClient
  class BrokenDocumentLink
    ...
    # Number of documents with this broken link.
    attr_accessor :documents

    # HTTP status for the broken link.
    attr_accessor :http_status

    # Timestamp for when the status of this link last changed.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SiteimproveAPIClient::BrokenDocumentLink` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SiteimproveAPIClient::BrokenDocumentLink`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @last_changed.nil?
        invalid_properties.push('invalid value for "last_changed", last_changed cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @last_changed.nil?
    ...
    def hash
      [id, url, documents, http_status, last_changed, last_checked, message, _links, _siteimprove].hash
    end
