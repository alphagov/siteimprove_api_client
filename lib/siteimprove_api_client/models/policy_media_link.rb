...
module SiteimproveAPIClient
  class PolicyMediaLink
    ...
    # The number of clicks on this media. This information is retrieved from the Analytics data, if available.
    attr_accessor :clicks

    # The number of documents referring to this media.
    attr_accessor :referring_documents

    # The number of pages referring to this media.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SiteimproveAPIClient::PolicyMediaLink` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SiteimproveAPIClient::PolicyMediaLink`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, url, clicks, referring_documents, referring_pages, violated_date, _links].hash
    end
