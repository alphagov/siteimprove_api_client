...
module SiteimproveAPIClient
  class HtmlValidationSeoPage
    ...
    # True if the html validation check is pending; otherwise false
    attr_accessor :pending

    # The page was validated at this time
    attr_accessor :validated_date

    # The number of w3 errors found on the page
    attr_accessor :w3_errors
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SiteimproveAPIClient::HtmlValidationSeoPage` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SiteimproveAPIClient::HtmlValidationSeoPage`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'w3_errors')
        self.w3_errors = attributes[:'w3_errors']
      end
    ...
    def hash
      [id, title, url, checking_now, cms_url, page_level, page_views, pending, validated_date, w3_errors, _siteimprove].hash
    end
