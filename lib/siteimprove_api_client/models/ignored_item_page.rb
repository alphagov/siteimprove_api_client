  ...
  class IgnoredItemPage
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SiteimproveAPIClient::IgnoredItemPage` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SiteimproveAPIClient::IgnoredItemPage`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @created_ts.nil?
        invalid_properties.push('invalid value for "created_ts", created_ts cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @created_ts.nil?
    ...
    def hash
      [id, title, url, cms_url, created_by, created_ts, note, subtype, _siteimprove].hash
    end
