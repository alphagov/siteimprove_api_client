...
module SiteimproveAPIClient
  class PageWithPolicies
    # The Id of this specific content matches.
    attr_accessor :id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SiteimproveAPIClient::PageWithPolicies` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SiteimproveAPIClient::PageWithPolicies`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @matching_policies.nil?
        invalid_properties.push('invalid value for "matching_policies", matching_policies cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @matching_policies.nil?
      true
    end
    ...
    def hash
      [id, title, url, cms_url, matching_policies, page_level, page_views, _links, _siteimprove].hash
    end
