...
module SiteimproveAPIClient
  class PageWithFeedback
    ...
    # Number of comments registered for the page.
    attr_accessor :comments

    # Number of page views registered.
    attr_accessor :page_views

    # Number of rating registered for the page.
    attr_accessor :ratings

    # Average satisfaction for the page.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SiteimproveAPIClient::PageWithFeedback` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SiteimproveAPIClient::PageWithFeedback`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, title, url, comments, page_views, ratings, satisfaction, _links, _siteimprove].hash
    end
