...
module SiteimproveAPIClient
  class CssValidationError
    ...
    # Is true if url is being checked now; otherwise false
    attr_accessor :checking_now

    # The number of css validation errors
    attr_accessor :errors

    # The number of pages the stylesheet is found on
    attr_accessor :pages
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SiteimproveAPIClient::CssValidationError` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SiteimproveAPIClient::CssValidationError`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, url, checking_now, errors, pages].hash
    end
