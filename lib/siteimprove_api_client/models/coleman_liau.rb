...
module SiteimproveAPIClient
  # Readability values for the \"Coleman Liau\" metric
  class ColemanLiau
    # Average number of letters or digits per 100 words
    ...
    def self.openapi_types
      {
        :'average_chars_with_digits' => :'Integer',
        :'average_sentences_with_digits' => :'Integer',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SiteimproveAPIClient::ColemanLiau` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SiteimproveAPIClient::ColemanLiau`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [average_chars_with_digits, average_sentences_with_digits, long_sentences_with_digits_unique, long_words_with_digits_unique].hash
    end
