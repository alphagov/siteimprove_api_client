...
module SiteimproveAPIClient
  ...
  class SummaryQualityAssurance1
    # Number of clicks on the broken link for this page.
    ...
    # Number of pages referring to this page.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SiteimproveAPIClient::SummaryQualityAssurance1` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SiteimproveAPIClient::SummaryQualityAssurance1`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [broken_links, misspellings, potential_misspellings, referring_pages].hash
    end
