...
module SiteimproveAPIClient
  class GroupDetails1
    ...
    # Number of broken links within this group.
    attr_accessor :broken_links

    # Name of the group.
    attr_accessor :group_name

    # Number of misspellings withing this group.
    attr_accessor :misspellings

    # Pages
    attr_accessor :pages

    # Number of potential misspellings within this group.
    attr_accessor :potential_misspellings

    # Number of users within the group.
    attr_accessor :users
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SiteimproveAPIClient::GroupDetails1` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SiteimproveAPIClient::GroupDetails1`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, broken_links, group_name, misspellings, pages, potential_misspellings, users, _links].hash
    end
