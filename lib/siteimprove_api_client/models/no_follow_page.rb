...
module SiteimproveAPIClient
  class NoFollowPage
    ...
    # The robot tag for the page
    attr_accessor :robot_tag
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SiteimproveAPIClient::NoFollowPage` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SiteimproveAPIClient::NoFollowPage`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'robot_tag')
        self.robot_tag = attributes[:'robot_tag']
      end
    ...
    def hash
      [id, title, url, checking_now, cms_url, page_level, page_views, robot_tag, _siteimprove].hash
    end
