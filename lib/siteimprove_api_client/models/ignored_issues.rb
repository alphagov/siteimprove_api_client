...
module SiteimproveAPIClient
  class IgnoredIssues
    # Id of note (if present).
    attr_accessor :id

    # The name of the user who ignored this SEO issue.
    attr_accessor :ignored_by

    # The time when this SEO issue was ignored.
    attr_accessor :ignored_time

    # The type of this issue.
    attr_accessor :issue_type

    # The number of SEO issues of this type that was ignored.
    attr_accessor :issues

    # Optional note which the user can fill out with a description of why this SEO issue was ignored.
    attr_accessor :note

    # Issue severity: error, warning, review.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SiteimproveAPIClient::IgnoredIssues` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SiteimproveAPIClient::IgnoredIssues`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'ignored_time')
        self.ignored_time = attributes[:'ignored_time']
      end
    ...
    def hash
      [id, ignored_by, ignored_time, issue_type, issues, note, severity, _links].hash
    end
