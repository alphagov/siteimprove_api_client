...
module SiteimproveAPIClient
  class PageForIgnoredIssueDecision
    # Decision page id
    attr_accessor :id
    ...
    # Whether the page is being checked now
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SiteimproveAPIClient::PageForIgnoredIssueDecision` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SiteimproveAPIClient::PageForIgnoredIssueDecision`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, title, url, checking_now, cms_url, created_by, created_ts, note, subtype, _siteimprove].hash
    end
