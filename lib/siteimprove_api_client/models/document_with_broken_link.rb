...
module SiteimproveAPIClient
  class DocumentWithBrokenLink
    # Id of the document with the broken link.
    attr_accessor :id

    # The URL of the document with the broken link.
    attr_accessor :url
    ...
    # Timestamp of when the document was last changed.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SiteimproveAPIClient::DocumentWithBrokenLink` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SiteimproveAPIClient::DocumentWithBrokenLink`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, url, broken_links, checking_now, last_modified, page_views, pages, _links, _siteimprove].hash
    end
