...
module SiteimproveAPIClient
  class SocialMediaOrganization
    # Id of organization.
    attr_accessor :id

    # Name of the social media organisation.
    attr_accessor :organisation

    # Number of pages the event was registered on.
    attr_accessor :pages

    # Number of referrals registered.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SiteimproveAPIClient::SocialMediaOrganization` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SiteimproveAPIClient::SocialMediaOrganization`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, organisation, pages, referrals, visits, _links].hash
    end
